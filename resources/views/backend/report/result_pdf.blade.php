        ...
        <div class="container mt-5">
            <div class="row">
                ...
                <div class="col-md-12 text-center">
                    <h5 style="font-weight: bold; color:black; padding-top:-25px; margin-right:70px;"><i>Results of {{ @$allData[0]['exam_type']['exam'] }}</i></h5>
                </div>
                <div class="col-md-12">        
                    ...
                    <table border="0" width="100%" cellpadding="1" cellspacing="2" class="text-center">
                        <tbody>
                            <tr>
                                <td><strong>Year/Session : </strong>{{ @$allData[0]['year_name']['year'] }}</td>
                                <td></td>
                                <td></td>
                                <td><strong>Class : </strong>{{ @$allData[0]['class_name']['class_name'] }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div class="col-md-12">
                    <table border="1" width="100%">
                        ...
                        <tbody>
                            ...
                            @php
                                $allMarks = App\Models\StudentMarks::where('year_id',$value->year_id)->where('class_id',$value->class_id)->where('exam_type_id',$value->exam_type_id)->
                                where('student_id',$value->student_id)->get();
                                $total_marks = 0;
                                $total_point = 0;
                                foreach ($allMarks as $data) {
                                    $count_fail = App\Models\StudentMarks::where('year_id',$data->year_id)->where('class_id',$data->class_id)->where('exam_type_id',$data->exam_type_id)->
                                where('student_id',$data->student_id)->where('marks','<','33')->get()->count();
                                $get_mark = $data->marks;
                            ...
                            @endphp
                                <tr>
                                    ...
                                    @php
                                        $total_subject = App\Models\StudentMarks::where('year_id',$value->year_id)->where('class_id',$value->class_id)->where('exam_type_id',$value->exam_type_id)->
