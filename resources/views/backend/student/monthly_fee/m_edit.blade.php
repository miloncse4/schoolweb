...
<div class="container">
    <div class="card mt-5">
      <div class="card-header bg-danger d-sm-flex align-items-center justify-content-between">
        <h2 class="text-light"><i><b>Edit Monthly Fee</b></i></h2>
        <a href="{{ route('student.monthly.view') }}" class="btn btn-sm btn-dark" style="font-weight:bold;">
      Back </a>
      </div>
      <div class="card-body">
        @if(session('InsErr'))
        ...
        @endif
        <form action="{{route('student.monthly.update',$data->id)}}" method="post">
            @csrf
        <div class="add_item">
            ...
            <div class="form-row">
                <div class="col-md-2 form-group">
                    <label class="text-dark">Fee name</label>
                        <input type="text" class="form-control" name="fee_name" value="{{ $data->fee_name }}">
                        @error('fee_name')
                        <small class="text-danger">{{$message}}</small>
                        @enderror
                         </div>
