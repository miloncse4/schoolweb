...
<div class="container">
    <div class="card mt-5">
      <div class="card-header bg-warning d-sm-flex align-items-center justify-content-between">
        <h2 class="text-light"><i><b>Promotion Student</b></i></h2>
        <a href="{{ route('student.reg.view') }}" class="btn btn-sm btn-light" style="font-weight:bold;">
        <i class="fa fa-bars"></i> Show Data</a>
      </div>
      <div class="card-body">
        @if(session('InsErr'))
        ...
        <table class="table table-bordered">
          <form action="{{route('student.reg.promotion.store',$editData->student_id)}}" method="post" enctype="multipart/form-data">
           @csrf
      ...
      <button type="submit" class="btn btn-warning text-light">Promotion</button>
          </form>
        </table>
      </div>
    </div>
